...
class Custom_Model extends Model
{
    ...
    {
//        DB::enableQueryLog();
        $sql = DB::table('districts')->select(db::raw("district_name ,dist_id as district_code"));
    ...
    public static function getAllUc()
    {
    ...
    public static function getAllHealth_facility()
    {
        $sql = DB::table('hf_list')->select('hf_list.id',
            'hf_list.distcode',
            'hf_list.uccode',
            'hf_list.hf_code',
            'hf_list.hf_name',
        ...
        $sql->where(function ($query) {
            $query->whereNull('hf_list.colflag')
    ...
    public static function genPassword2($password, $salt)
    {
        $algorithm = 'sha512';
        $count = 1000;
        $key_length = (64 * 8);
    ...
    public static function getDistrictArray()
    {
        return [
            '101' => 'rajanpur',
            '201' => 'dadu'
    ...
    public static function getUserDistrct($user_districts)
    {
        foreach ($user_districts as $d){
            if($d=='201'){
                return trim($d->distcode);
            }else if($d=='101'){
                return trim($d->distcode);
            }
        }
